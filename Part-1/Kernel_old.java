...
public class Kernel {
	...
	public static int interrupt(int irq, int cmd, int param, Object args) {
		...
		case INTERRUPT_SOFTWARE: // System calls
			...
			case WAIT:
				myTcb = scheduler.getMyTcb(); // get current TCB
				// get the current thread id
				// check the current TCB is null or not
				int thread_id = (myTcb != null) ? myTcb.getTid() : ERROR;
				// let the current thread sleep in waitQueue under the
				// condition = this thread id
				return waitQueue.enqueueAndSleep(thread_id); // return a child thread id who woke me up
			case EXIT:
				myTcb = scheduler.getMyTcb();
				// get the current thread's parent id and current thread id
				// check the current TCB is null or not
				int parent_thread_id = (myTcb != null) ? myTcb.getPid() : ERROR;
				int thread_id_2 = (myTcb != null) ? myTcb.getTid() : ERROR;
				// search waitQueue for and wakes up the thread under the
				// condition = the current thread's parent id
				waitQueue.dequeueAndWakeup(parent_thread_id, thread_id_2);
